...
int mitsuba_app(int argc, char **argv) {
	...
	try {
		...
		while ((optchar = getopt(argc, argv, "a:c:D:s:j:n:o:r:b:p:L:qhzvtwxO")) != -1) {
			switch (optchar) {
				case 'a': {
		...
		for (int i=optind; i<argc; ++i) {
            ...
            if (exportObj) {
                SLog(EInfo, "Exporting scene meshes to Wavefront OBJ...");
                for(auto& m: scene->getMeshes()) {
                    auto objName = m->getName() + ".obj";
                    if (objName == "unamed.obj") {
                        SLog(EError, "Unnamed object: Please provide an ID to mesh in scene file");
                    }
                    auto fullPath = scene->getSourceFile().parent_path() / objName;
                    SLog(EInfo, "Exporting %s to %s", m->getName().c_str(), fullPath.c_str());
                    m->writeOBJ(fullPath);
                }
                return 0;
            }
