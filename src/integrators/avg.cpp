...
public:
  AvgIntegrator(const Properties &props) : Integrator(props) {
    ...
    m_dumpIteration = props.getInteger("dumpIteration", 1); // Every 5 frames
...
//    int blockSize = scene->getBlockSize();
    ...
    // Stop condition for the average integrator...
    auto stop_condition = [this, &cumulativeTime](int it) -> bool {
        if(m_maxPasses == -1 && m_maxRenderingTime == -1) {
            return !m_stop;
        }
        if(m_maxPasses != -1 && it >= m_maxPasses) {
            return false;
        }
        if(m_maxRenderingTime != -1 && cumulativeTime >= m_maxRenderingTime) {
            return false;
        }
        return !m_stop;
    };

    for(int it = 1; stop_condition(it); it++) {
